...
function Complain() {
  ...
  const handleClickOpen = (a) => {
    ...
    for (var i = 0; i <= complains.length; i++) {
      // console.log(complains[i]._id)
      if (complains[i]._id === id) {
        // console.log(complains[i]._id)

        setShikayat(complains[i].Complain)
        console.log(shikayat)
      }
    }
  ...
  const leftDiv = (i) => {
    ...
    return (
      <Paper key={i._id} elevation={5} className='paper' onClick={() => handleClickOpen(i._id)}>
  ...
  return (
    <>
      <h1 className='headingh1'>COMPLAIN</h1>
      <div className='i'>
